...
function LogBook() {
    ...
    const [assignment, setAssignment] = useState<forms>({formID: "", type: "", location: "", status: "", assignee: "", dateCreated: emptyDate});
    ...
    // useEffect(() => {
    //     axios.get("/api/csvManager/").then((response) => {
    //         setFormID(response.data.reverse());
    //     });
    // }, []);
            ...
            {/*Form to assign requests*/}
                <div className="rounded-2xl bg-deep-blue bg-opacity-5">
                    <form ref = {formRef} onSubmit={e => {e.preventDefault();}}
                        ...
                        <p className={"text-left font-bold"}>Request Status</p>
                        <Dropdown options={statusTypeOptions} placeholder={"Choose Status"} name={"statusAssignment"}
