...
export function AlgorithmButtons(props: {href?: string, onClick?:(e: React.MouseEvent) => void, px: string, children?: ReactNode, isActive: boolean}) {
    ...
    const inactiveClass = "w-0 h-1 bg-deep-blue transition-all group-hover:w-[calc(50%+16px)]";

    return <div className={`${props.px}`}>
        <p className="py-1 relative group font-OpenSans items-center font-bold text-deep-blue ">
